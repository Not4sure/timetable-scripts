...
#         res = range(2, 15, 2) if '2-14' in week else range(1, 16, 2) if 'н/п' in week else range(1, 10) if '1-9' in week else range(1, 16)
    ...
    if is_not_blank(week):
        res = 'even' if 'пар' in week else 'odd' if 'н/п' in week else 'all'
...
wb = openpyxl.load_workbook('routine-2.xlsx')
sheet_name = '1 курс'
sheet = wb[sheet_name]
...
for col in list(string.ascii_uppercase):
    for row in range(16, 200):
        ...
        if is_blank(value):
            continue
        value = str(value)
        if '4 - 18' in value or '5 - 17' in value or '1-9' in value:
            lesson = {}
...
#             lesson['type'] = get_type(res)
            ...
            for i in range(1, 7):
                ...
                if is_not_blank(subject):
                    ...
                    if '5 - 17' in value_v or '4 - 18' in value_v or '1-9' in value_v:
                        break
                    ...
                    if is_not_blank(subject):
                        ...
                        type = get_cell_value(sheet, cell)

            lesson['subject'] = subject
#             lesson['room'] = room
            lesson['type'] = get_type(type)
